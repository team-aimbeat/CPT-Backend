...
class TranscodeWarmupVideo implements ShouldQueue
{
    use Dispatchable, InteractsWithQueue, Queueable, SerializesModels;

    public $warmupVideoId;
    public $sourcePath;

    public function __construct($warmupVideoId, $sourcePath)
    {
        $this->warmupVideoId = $warmupVideoId;
    ...
    public function handle()
    {
        ...
        WarmupVideo::where('id', $this->warmupVideoId)->update([
            'transcoding_status' => 'processing',
        ]);

        try {
            $disk = Storage::disk('s3');
            $tempDir = storage_path('app/temp/warmup_videos/' . $this->warmupVideoId);
            if (!is_dir($tempDir)) {
