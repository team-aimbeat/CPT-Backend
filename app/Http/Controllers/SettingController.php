...
class SettingController extends Controller
{
    ...
    public function settingUpdate(Request $request)
    {
        if(env('APP_DEMO')){
            ...
            return redirect()->route('setting.index', ['page' => 'mobile-config'])->withErrors($message);
        }
        $data = $request->all();
    ...
    public function settingsUpdates(Request $request)
    {
        ...
        if(env('APP_DEMO')){
            $message = __('message.demo_permission_denied');
            return redirect()->route('setting.index', ['page' => $page])->withErrors($message);
        }
    ...
    public function updateProfile(Request $request)
    {
        if(env('APP_DEMO')){
            $message = __('message.demo_permission_denied');
            return redirect()->route('setting.index', ['page' => 'profile-form'])->withErrors($message);
        }
        $user = Auth::user();
        $data = $request->all();
    ...
    public function changePassword(Request $request)
    {
        if(env('APP_DEMO')){
            $message = __('message.demo_permission_denied');
            return redirect()->route('setting.index', ['page' => 'password-form'])->withErrors($message);
        }
    ...
    public function saveTermAndCondition(Request $request)
    {
        if(env('APP_DEMO')){
            $message = __('message.demo_permission_denied');
            return redirect()->route('pages.term_condition')->withErrors($message);
        }
        $setting_data = [
    ...
    public function savePrivacyPolicy(Request $request)
    {
        if(env('APP_DEMO')){
            $message = __('message.demo_permission_denied');
            return redirect()->route('pages.privacy_policy')->withErrors($message);
        }
        $setting_data = [
                        'type'   => 'privacy_policy',
                        'key'   =>  'privacy_policy',
    ...
    { 
        if(env('APP_DEMO')){
            $message = __('message.demo_permission_denied');
            return redirect()->route('setting.index',['page' => 'payment-setting'])->withErrors($message);
        }
        if(!auth()->user()->hasRole('admin')) {
            abort(403, __('message.action_is_unauthorized'));
        }
