                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="form-group col-md-4">
                                {{ Form::label('workout_modes', __('message.workouttype').' <span class="text-danger">*</span>',[ 'class' => 'form-control-label' ], false) }} 
                                {{ Form::select('workout_modes[]', 
                                        [ 
                                        'gym' => 'Gym workout' ,
                                        'home' => 'Home exercise',
                            ...
                            <div class="form-group col-md-4 video_upload">
                                <label class="form-control-label" for="equipment_video">{{ __('message.video') }}
                                </label>
                                <div class="">
                                    <input class="form-control file-input" type="file" name="equipment_video"
                                        accept="video/*" id="equipment_video" />
                                </div>
                            </div>
                            @if( isset($id) && getMediaFileExit($data, 'equipment_video'))
                            <div class="col-md-2 mb-2 position-relative">
                                <?php
                                        $file_extention = config('constant.IMAGE_EXTENTIONS');
                                        $image = getSingleMedia($data, 'equipment_video');
                                        
                                        $extention = in_array(strtolower(imageExtention($image)), $file_extention);
                                    ?>
                                @if($extention)
                                <img id="equipment_video_preview" src="{{ $image}}" alt="equipment-video"
                                    class="avatar-100 mt-1" />
                                @else
                                    <img id="equipment_video_preview" src="{{ asset('images/file.png') }}" class="avatar-100" />
                                    <a href="{{ $image }}" download>{{ __('message.download') }}</a>
                                @endif
                                <a class="text-danger remove-file"
                                    href="{{ route('remove.file', ['id' => $data->id, 'type' => 'equipment_video']) }}"
